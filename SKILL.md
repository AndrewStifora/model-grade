...
- `subagent_type`: `mg-run-<effort>` for Sonnet, Opus, and Fable verdicts. These executors live in `~/.claude/agents/` and pin the effort level: `mg-run-low`, `mg-run-medium`, `mg-run-high`, `mg-run-xhigh`, `mg-run-max`. For a Haiku verdict use `general-purpose` (Haiku has no effort parameter).
